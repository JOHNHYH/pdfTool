...
public class PdfToWordPerfect {

    ScheduledExecutorService se = null;

    public PdfToWordPerfect(ScheduledExecutorService se) {
        this.se = se;
    }
...
//    String splitPath = "./split/";
    String splitPath = System.getProperty("user.dir") + File.separator + "split" + File.separator;

    // 3、如果是大文件，需要对子pdf文件一个一个进行转化
    String docPath = System.getProperty("user.dir") + File.separator + "doc" + File.separator;
        ...
        try {
            ...
            if (flag && flag1) {
                ...
                // 3、否则输入的页数比较多，就开始进行切分再转化
                    ...
                    CountDownLatch latch = new CountDownLatch(fs.length);
                    for (int i=0;i<fs.length;i++) {
                        PdfDocument sonpdf = new PdfDocument();
                        se.execute(new SaveToFileTask(fs[i], sonpdf, latch));
                    }
                    latch.await();
    ...
    class SaveToFileTask implements Runnable {

        private File file;

        private PdfDocument sonpdf;

        private CountDownLatch countDownLatch;

        public SaveToFileTask(File file, PdfDocument sonpdf,CountDownLatch countDownLatch) {
            this.file = file;
            this.sonpdf = sonpdf;
        ...
        public void run() {
            sonpdf.loadFromFile(file.getAbsolutePath());
            sonpdf.saveToFile(docPath + file.getName().substring(0, file.getName().length() - 4) + ".docx", FileFormat.DOCX);
